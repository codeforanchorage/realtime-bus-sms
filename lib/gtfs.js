        ...
        var curl_process = exec(`curl ${muni_URL}${gtfs_file} -s -w "%{http_code}" -z "${raw_directory}${gtfs_file}" -o "${raw_directory}${gtfs_file}"`, (err, stdout, stderr) => {
            ...
            switch (stdout) {
                ...
                case "200":
                    ...
                    var zip_process = exec(`unzip -o "${raw_directory}${gtfs_file}"  -d "${raw_directory}"`, (err, stdout) => {
                        if (err) {
                            logger.error(err);
                            reject(err);
                        } else {
...
const GTFS_Check = new EventEmitter()
...
logger.debug("Checking that GTFS is in place ")

fs.ensureDir(raw_directory)
.then(getGTFSFile)
.then(() => all_stops.init())
.then(() => exceptions.init())
.then(() => routes.init())
.then(() => {
    logger.debug("GTFS checks okay")
    GTFS_Check.emit("ready")
})
.catch(err => {
    logger.error(err)
})
