...
/*
* Request gtfs zip file and process 
*/

getGTFSFile(); // this should be called on a schedule to check for changes.
...
function getGTFSFile() {
    ...
    var curl_process = exec(`curl ${muni_URL}${gtfs_file} -s -w "%{http_code}" -z "${raw_directory}${gtfs_file}" -o "${raw_directory}${gtfs_file}"`, (err, stdout, stderr) => {
        ...
        switch (stdout) {
            case "304":
                logger.warn("Attempted to download muni GTFS file. File unchanged.");
                break;
            case "200":
                logger.warn("New GTFS file downloaded")
