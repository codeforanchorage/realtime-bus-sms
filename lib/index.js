...
function parseInputReturnBusTimes(message, callback, returnHtml) {
    function resultsHandler(err, data) {
        ...
        callback(err, output, geocodedAddress, null, returnHtml);
    }

    if (message.trim().toLowerCase() === 'about') {
        return resultsHandler(null, 'Get bus ETAs, text the busstop #. Don\'t know the busstop #, text closest street address or cross streets and get up to 5 closest stops within a mile, text back desired bustop #. Text \'Feedback: ..\' to send feedback. Made by Code for Anchorage');
    }
