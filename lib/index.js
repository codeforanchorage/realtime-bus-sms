...
function stopLatLong(stopid) {
    ...
    for (var i=0; i < all_stops.features.length; i++) {
        if (all_stops.features[i].properties.stop_id.split(" ")[0] == stopid) {
...
//        return stop.properties.name + ' (stop: ' + stop.properties.stop_id + ')';
        return { route: stop.properties.name + ': ' + stop.properties.stop_id,
            stopId: stop.properties.stop_id }
    });

    if (out.length < 1) {
        out = 'No stops found within a 1/4 mile';
    }
