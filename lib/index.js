...
function prependRouteNumberToRouteName(routeName) {
    ...
    var routeNumber = gtfs.routeNamesToRouteNumbers[nameOnly];
    if (routeNumber) {
...
function stopLatLong(stopid) {
    for (var i=0; i < gtfs.all_stops.features.length; i++) {
        if (gtfs.all_stops.features[i].properties.stop_id.match(/\d+/g)[0] == stopid) {
            return gtfs.all_stops.features[i].geometry.coordinates[1] + "," + gtfs.all_stops.features[i].geometry.coordinates[0];
        }
    }
    return "";
}
...
function findNearestStops(lat, lon) {
    ...
    var out = gtfs.all_stops.features.reduce((acc, stop) => {
...
function serviceExceptions() {
    ...
    for(var i=0; (i < gtfs.exceptions.length) && !replacementService; i++) {
        if (gtfs.exceptions[i].date == dateTz) {
            if (gtfs.exceptions[i].exception_type == 1) {
