...
function parseInputReturnBusTimes(message, callback) {
    ...
    if (message.trim().toLowerCase() === 'about') {
        return resultsHandler(null, 'To get bus ETAs, text the bus stop #. If you do not know the bus stop id, text the closest street address or closest cross streets to the bus stop.');
    }
        ...
        getStopFromAddress(message, resultsHandler)
    }
}
