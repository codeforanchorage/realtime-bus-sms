...
function getStopFromStopNumber(stopId) {
    ...
    .catch((err) => { logger.warn("Error requesting bus data: ", err);
...
*/
 ...
 function getStopsFromAddress(address){
    ...
    return exports.getGeocodedAddress(addressWithCity)
    .then((returnObj) => {
        geocodedAddress = returnObj.data
        if (!geocodedAddress || !geocodedAddress.streetName) { 
        ...
        if (stops.length < 1) {
            return Promise.reject({
                name: "No Stops", 
                message: `No stops found within ${config.NEAREST_BUFFER} mile` + ((config.NEAREST_BUFFER != 1) ? 's' : ''),
                type:'NOT_FOUND'
            });
        }      
...
function getGeocodedAddress(address) {
    ...
    return geocoder.geocode(address)
        ...
        .catch((e) => logger.warn("Error in geocoder: " + e));
}
...
function processFeedback(feedback, req) {
    return new Promise(function(resolve, reject){
        ...
        comments.get('comments').push({ date: (new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '')),
                                    feedback: feedback,
                                    phone: (req.locals && req.locals.isFB) ? undefined : req.body.From,
                                    fbUser: (req.locals && req.locals.isFB) ? req.body.From : undefined,
