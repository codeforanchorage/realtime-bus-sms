...
*/
 ...
 function getStopsFromAddress(address){
    ...
    .then((returnObj) => {
        ...
        var geocodedCoords = geocodedPlace.geometry.location // this can be defined even when address isn't found
        var address = geocodedPlace.formatted_address
        var lat = geocodedCoords.lat;
        var lon = geocodedCoords.lng;
        ...
        request(`${GEOCODE_URL_BASE}address=${querry}&components=country:${COUNTRY}|administrative_area:${CITY}&key=${process.env.GOOGLE_MAPS_KEY}`, function(error, response, body){
            if (!error && response.statusCode == 200) {
                ...
                if (geocodeData.status != "OK") { 
                    return reject(`Geocoder Error: ${geocodeData.status}`)
                }
                ...
                if (result.types && acceptable_types.some(el => result.types.includes(el))){
                    resolve({data: result, asyncTime: Date.now()-timer})
                } else {
                    resolve({data: null, asyncTime: Date.now()-timer})
                }

            } else {
                reject("Network Error Reaching Geocoder: ", error)
            }
        })
