...
var GAC_CLIENT_ID = "UA-56999250-2";
...
function consoleFormatter(options){
    ...
    // The logger that writes all requests doesn't send a message, only meta data
    var meta = options.meta;

    var logMessage = options.message 
                     ? winston.config.colorize(options.level) + `: ${options.message}` + (options.meta && Object.keys(options.meta).length ? JSON.stringify(options.meta) : '' )
...
// THis needs a way to identify new and returning users - maybe hash of phone number + coookie UUID for web? */ 
