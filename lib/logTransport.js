...
/* Things to log that aren't currently:
  feedback
  on web differentiate between direct url hit (ie. 'find/0235') and ajax request
  404 and 5xx errors
  needs a way to identify new and returning users - hash of phone number + coookie UUID for web? */ 
...
GATransport.prototype.log = function(level, msg, meta, callback){
    ...
    .send()
    logger.debug("event: ", category, action, label, meta.responseTime, meta.muniTime)
}
...
function log(req, res, next){
    ...
    logger.debug("action: " res.locals.action)
