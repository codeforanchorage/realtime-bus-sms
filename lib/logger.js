 ...
 * logger.error(Error)
 * logger.error({message:message, meta: {key: value}})
...
class RollbarTransport extends Transport{
    ...
    log(info, callback) {
        if (info instanceof Error) {     
            // called logger.error(new Error("there was an error"))           
            rollbar.error(info, callback)
        } else if (info.message instanceof Error) {
            // called logger.error(new Error("there was an error"), {somekey: "Some Value"})  
            let {level, message, ...rest} = info
            rollbar.error(message, null, rest, callback)
        } else {
            // called logger.error({message:"some string", meta: {optional: 'object data'}})
            let {level, message, ...rest} = info
            rollbar.error(message, rest, callback)
        }
        callback()    
    }
}
...
function initGoogleAnalytics(initFunction){
    ...
    class GATransport extends Transport{
        ...
        log(info, callback){
            ...
            if (info.method == "HEAD") return callback();
    
            const _fields = initFunction(info) || {}
            // Don't send warnings and debug messages to google analytics
            if (info.level != 'info') return callback();

            //  To distinguish new and returning users
            const uuid = _fields.uuid || info.uuid;
            if (!uuid){
               callback()
               return logger.warn("Could not make a UUID for Google Analytics")
            }
            ...
            if (!_fields.action) {
                visitor.pageview(info.url, (err, res) => {
                    if (err) {
            ...
            visitor.timing('Response Time', 'Total Time', info.responseTime )
            if (_fields.timings){
