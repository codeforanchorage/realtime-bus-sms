...
function transport(public_file = db_path, private_file = db_private_path ){
    ...
    const db = new FileASync(public_file)
    const db_private = new FileASync(private_file)

    class lowdbTransport extends Transport {
        constructor(opts) {
            super({
                name: "File-Logs",
        ...
        log(level, msg, meta, callback){
            ...
            return Promise.all([
                low(db_private)
                .then(handle => handle.defaults({ requests: []}) .get('requests').push(entry).write()),
                low(db)
                .then(handle => handle.defaults({ requests: []}).get('requests').push(entry2).write())
...
function getLogData(daysBack, type, private_file = db_private_path){
    ...
    return low(db_private).then( db_handle => {
        const logData = [];
        const timezone = moment.tz.zone(config.TIMEZONE);
        if (type == "hits") {
            const data = db_handle.get('requests').value();
            let dateTz = null;
