...
function transport(public_file = './public/db.json', private_file = './db_private.json' ){
    ...
    //   which is especially handy for tests.
      ...
      lowdbTransport.prototype.log = function(level, msg, meta, callback) {
        ...
        if (this.silent) return // allow silent mode - nice for running tests without writing logs
        ...
        db_private.defaults({ requests: []}) .get('requests').push(entry).write() //write is asyncchronous
        var entry2 = Object.assign({}, entry);         // so we copy the object to avoid overwriting it before write
        if (entry.phone) {
