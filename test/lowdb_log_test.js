...
describe('LowDB Log Transport', function(){
    ...
    it('Should save ip and phone to private db', function(){
        ...
        var saved = JSON.parse(stenoStubb.firstCall.args[1]).requests.pop()
    ...
    it('Should save hashed FB user to public db', function(){
        logger.info(metaObject)
        var saved = JSON.parse(stenoStubb.secondCall.args[1]).requests.pop()
    ...
    it('Should save private db to correct file', function(){
        var private_db_file  = './db_private.json'
        logger.info(metaObject)
        var path = stenoStubb.firstCall.args[0]
        assert.equal(private_db_file, path)
    })
    it('Should save public db to correct file', function(){
        var public_db_file  = './public/db.json'
        logger.info(metaObject)
        var path = stenoStubb.secondCall.args[0]
        assert.equal(public_db_file, path)
    })
    it('Should save a timestampe of the current date', function(){
        var clock = sinon.useFakeTimers(1483228800000) // First milisecond of 2017
        logger.info(metaObject)
        var saved = JSON.parse(stenoStubb.secondCall.args[1]).requests.pop()
...
describe('Get Log Data', function(){
    ...
    let clock,
        fixturePath = path.resolve(__dirname, 'fixtures/fake_low_db.json')
        fixtureFile = require(fixturePath)
    beforeEach(function(){
    ...
    it('Should return only hits within given time frame', function(){
        var daysBack = 1
        var dateThen = new Date().setDate(new Date().getDate()-daysBack)
        var data = lowdb_log.getLogData(daysBack, 'hits',fixturePath )
    ...
    it('Should set the correct userID', function(){
        var daysBack = 0
        var dateThen = new Date().setDate(new Date().getDate()-daysBack)
        var data = lowdb_log.getLogData(0, 'hits',fixturePath )
    ...
    it('Returned objects should have the correct properties', function(){
        var properties = ['type', 'date', 'dateOffset', 'muniTime', 'totalTime', 'userId']
        var daysBack = 10
        var dateThen = new Date().setDate(new Date().getDate()-daysBack)
        var data = lowdb_log.getLogData(0, 'hits',fixturePath )
